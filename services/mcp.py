...
def get_mcps():
    if STREAMLIT_MODE == "api":
        response = httpx2.get(f"{API_BASE_URL}/api/mcp")
        items = handle_response(response)
        for item in items:
            if item["type"] == "Real File Upload":
                dl_resp = httpx2.get(f"{API_BASE_URL}/api/mcp/{item['id']}/download")
                if dl_resp.status_code == 200:
                    item["file_blob"] = dl_resp.content
        return items
    else:
        from core.mcp import get_mcps as core_get
        return core_get()

def get_mcp_by_id(item_id):
    if STREAMLIT_MODE == "api":
        response = httpx2.get(f"{API_BASE_URL}/api/mcp/{item_id}")
        item = handle_response(response)
        if item["type"] == "Real File Upload":
            dl_resp = httpx2.get(f"{API_BASE_URL}/api/mcp/{item_id}/download")
            if dl_resp.status_code == 200:
                item["file_blob"] = dl_resp.content
        return item
    else:
        from core.mcp import get_mcp_by_id as core_get_by_id
...
def create_mcp(name, storage_type, content, file_blob, file_name, description, tags, reference_url=""):
    if STREAMLIT_MODE == "api":
        ...
        response = httpx2.post(f"{API_BASE_URL}/api/mcp", json=payload)
        data = handle_response(response)
        return data["id"]
    else:
        from core.mcp import create_mcp as core_create
...
def update_mcp(item_id, name, storage_type, content, description, tags, reference_url="", file_blob=None, file_name=None):
    if STREAMLIT_MODE == "api":
        ...
        response = httpx2.put(f"{API_BASE_URL}/api/mcp/{item_id}", json=payload)
        data = handle_response(response)
        return data["id"]
    else:
        from core.mcp import update_mcp as core_update
        return core_update(item_id, name, storage_type, content, description, tags, reference_url, file_blob, file_name)

def delete_mcp(item_id):
    if STREAMLIT_MODE == "api":
        response = httpx2.delete(f"{API_BASE_URL}/api/mcp/{item_id}")
        handle_response(response)
    else:
        from core.mcp import delete_mcp as core_delete
        core_delete(item_id)
