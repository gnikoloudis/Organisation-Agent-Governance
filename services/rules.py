...
def get_rules():
    if STREAMLIT_MODE == "api":
        response = httpx2.get(f"{API_BASE_URL}/api/rules")
        items = handle_response(response)
        for item in items:
            if item["type"] == "Real File Upload":
                dl_resp = httpx2.get(f"{API_BASE_URL}/api/rules/{item['id']}/download")
                if dl_resp.status_code == 200:
                    item["file_blob"] = dl_resp.content
        return items
    else:
        from core.rules import get_rules as core_get
        return core_get()

def get_rule_by_id(item_id):
    if STREAMLIT_MODE == "api":
        response = httpx2.get(f"{API_BASE_URL}/api/rules/{item_id}")
        item = handle_response(response)
        if item["type"] == "Real File Upload":
            dl_resp = httpx2.get(f"{API_BASE_URL}/api/rules/{item_id}/download")
            if dl_resp.status_code == 200:
                item["file_blob"] = dl_resp.content
        return item
    else:
        from core.rules import get_rule_by_id as core_get_by_id
...
def create_rule(name, storage_type, content, file_blob, file_name, description, tags, reference_url=""):
    if STREAMLIT_MODE == "api":
        ...
        response = httpx2.post(f"{API_BASE_URL}/api/rules", json=payload)
        data = handle_response(response)
        return data["id"]
    else:
        from core.rules import create_rule as core_create
...
def update_rule(item_id, name, content, description, tags, reference_url="", file_blob=None, file_name=None):
    if STREAMLIT_MODE == "api":
        ...
        response = httpx2.put(f"{API_BASE_URL}/api/rules/{item_id}", json=payload)
        data = handle_response(response)
        return data["id"]
    else:
        from core.rules import update_rule as core_update
        return core_update(item_id, name, content, description, tags, reference_url, file_blob, file_name)

def delete_rule(item_id):
    if STREAMLIT_MODE == "api":
        response = httpx2.delete(f"{API_BASE_URL}/api/rules/{item_id}")
