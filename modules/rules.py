                ...
                with st.container(border=True):
                    ...
                    with st.expander("🔗 Related References, Assets & Tools"):
                        ...
                        with tab_link_url:
                            ...
                            # Button to fetch
                            if st.button("Fetch & Pre-fill URL Content", key=f"btn_fetch_url_Rules_{item['id']}"):
                                if not import_url.strip():
                                    st.error("Please enter a URL first.")
                                else:
                                    try:
                                        fetched_data = fetch_remote_content(import_url)
                                        st.session_state[f"fetched_content_Rules_{item['id']}"] = fetched_data
                                        ...
                                        if not import_rel_alias:
                                            st.session_state[f"import_alias_Rules_{item['id']}"] = filename_from_url
                                        if not child_name:
                                            st.session_state[f"import_name_Rules_{item['id']}"] = filename_from_url.split(".")[0]
                                            
                                        # If markdown
                                        if filename_from_url.endswith(".md"):
                                            st.session_state[f"import_storage_Rules_{item['id']}"] = "Markdown Text"
                                            fetch_name, fetch_desc, fetch_tags, _ = parse_frontmatter(fetched_data)
                                            if fetch_name: st.session_state[f"import_name_Rules_{item['id']}"] = fetch_name
                                            if fetch_desc: st.session_state[f"import_desc_Rules_{item['id']}"] = fetch_desc
                                            if fetch_tags: st.session_state[f"import_tags_Rules_{item['id']}"] = fetch_tags
                                        else:
                                            st.session_state[f"import_storage_Rules_{item['id']}"] = "Real File Upload"
                                ...
                                if st.button("Save & Link Imported Item", key=f"btn_save_imported_Rules_{item['id']}", type="primary"):
                                    try:
                                        ...
                                        if child_type == "Real File Upload":
                                            ...
                                            child_new_id = create_rule(child_name, child_type, "", file_blob, file_name, child_desc, child_tags)
                                        else:
