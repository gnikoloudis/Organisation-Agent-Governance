                ...
                with st.container(border=True):
                    ...
                    with st.expander("🔗 Related References, Resources Assets & Tools"):
                        ...
                        if relations:
                            ...
                            for rel in relations:
                                ...
                                with st.container(border=True):
                                    ...
                                    with col_c2:
                                        if st.button("🗑️ Delete Link", key=f"del_link_Skills_{item['id']}_{child_asset['id']}_{rel['relation_type']}_{rel['relation_alias']}"):
                                            try:
                                    ...
                                    if child_asset['type'] == "Real File Upload" and child_asset.get('file_blob'):
                                        st.download_button(
                                            ...
                                            key=f"dl_rel_Skills_{item['id']}_{child_asset['id']}_{rel['relation_type']}_{rel['relation_alias']}"
                                        )
                                        
                                    # Edit relation details
                                    with st.expander("✏️ Edit Relation Details"):
                                        c_name_key = f"edit_name_rel_Skills_{item['id']}_{child_asset['id']}_{rel['relation_type']}_{rel['relation_alias']}"
                                        c_desc_key = f"edit_desc_rel_Skills_{item['id']}_{child_asset['id']}_{rel['relation_type']}_{rel['relation_alias']}"
                                        c_text_key = f"edit_content_rel_Skills_{item['id']}_{child_asset['id']}_{rel['relation_type']}_{rel['relation_alias']}"
                                        c_tags_key = f"edit_tags_rel_Skills_{item['id']}_{child_asset['id']}_{rel['relation_type']}_{rel['relation_alias']}"
                                        
                                        c_type_key = f"edit_type_rel_Skills_{item['id']}_{child_asset['id']}_{rel['relation_type']}_{rel['relation_alias']}"
                                        c_alias_key = f"edit_alias_rel_Skills_{item['id']}_{child_asset['id']}_{rel['relation_type']}_{rel['relation_alias']}"
                                        ...
                                        if child_asset['type'] in ["Markdown Text", "Real File Upload"]:
                                            ...
                                            if child_asset['type'] == "Real File Upload":
                                                ...
                                                c_new_file = st.file_uploader("Upload New File to Replace", key=f"edit_file_rel_Skills_{item['id']}_{child_asset['id']}_{rel['relation_type']}_{rel['relation_alias']}")
                                                if c_new_file is not None:
                                                    c_new_file_bytes = c_new_file.read()
                                                    c_new_file_name = c_new_file.name
                                        elif child_asset['type'] == "Web Bookmark":
                                            c_new_content = st.text_input("URL", value=child_asset['content'], key=f"edit_url_rel_Skills_{item['id']}_{child_asset['id']}_{rel['relation_type']}_{rel['relation_alias']}")
                                            
                                        if st.button("💾 Save Changes", key=f"save_edit_rel_Skills_{item['id']}_{child_asset['id']}_{rel['relation_type']}_{rel['relation_alias']}", type="secondary"):
                                            try:
                                                ...
                                                from services.skills import update_skill_relation
                                                update_skill_relation(
                                                    skill_id=item['id'],
                        ...
                        with tab_link_existing:
                            candidates = []
                            try:
                                candidates = [c for c in get_skills() if c['id'] != item['id']]
                                candidates.sort(key=lambda x: x['name'].lower())
                            except Exception:
                                pass
                                
                            if not candidates:
                                st.info("No other Skills available to link.")
                            else:
                                selected_candidate_name = st.selectbox(
                                    "Select Skill to link",
                                ...
                                rel_type = st.selectbox("Relation Type", ["reference", "asset", "tool","resources","scripts"], key=f"cand_type_Skills_{item['id']}")
                        ...
                        with tab_link_url:
                            ...
                            # Relation fields
                            import_rel_type = st.selectbox("Relation Type", ["reference", "asset", "tool","resources","scripts"], key=f"import_type_Skills_{item['id']}")
